...
-- Spawn basic targets and weapon pickups to validate combat.
...
-- Initialize the room, player, world entities, input, and turn manager.
function love.load()
  love.window.setTitle("Deimos - Prototype")
  love.graphics.setBackgroundColor(0.04, 0.04, 0.04)

  local room = Room:new({ tileSize = 32, tilesWide = 18, tilesHigh = 12 })
  ...
  player = Player:new({ size = 24, speed = 220, equippedWeaponId = "fists" })
  ...
  -- Helper factory so player can spawn dropped pickups without coupling imports.
...
-- Draw world geometry, entities, pickups, and HUD.
function love.draw()
  world.room:draw()
  ...
  love.graphics.print("Déplacement: Flèches/WASD", 24, 8)
